                ...
                if st.session_state.last_img == img_id:
                    ...
                    with st.spinner(f"正在呼叫 {MODEL_NAME} 视觉大模型执行工艺数据比对，请稍候..."):
                        try:
                            ...
                            # 核心提示词：标准DCS读取 + 灵活模板映射
                            prompt = f"""
                            你是一个资深的化工DCS巡检员。请从这张中控截图中提取数据，填入Excel模板。

                            【DCS读取规则（固定）】
                            这是SUPCON双管反应釜界面，左侧是a管（主釜），右侧是b管（次釜）。
                            只读PV实际值（绿底黑字的大号数字），不读SP设定值（旁边的小号数字）。

                            请精确读取以下PV值（单位不填）：
                            流量类（在管路旁的数值框中）：
                              金属液a=___, 金属液b=___
                              液碱a=___, 液碱b=___
                              氨水a=___, 氨水b=___（氨水有两个入口，分别读取，可能一个为0）
                              氮气a=___, 氮气b=___
                              空气a=___, 空气b=___

                            主釜数据面板（左侧下方）：
                              pH=___（有两个通道，取较小值，忽略15左右的）
                              主釜温度=___, 主釜转速=___, 主釜液位=___

                            次釜数据面板（左侧下方次釜区域）：
                              次釜温度=___, 次釜转速=___, 次釜液位=___

                            【模板映射规则】
                            模板列名（按顺序）：
                            {formatted_columns}

                            映射方式：逐个检查模板列名，按以下关键词匹配：
                            - 含"金属液"且含"a"或"左"→金属液a的值
                            - 含"金属液"且含"b"或"右"→金属液b的值
                            - 含"液碱"且含"a"或"左"→液碱a的值
                            - 含"液碱"且含"b"或"右"→液碱b的值
                            - 含"氨水"或"铝碱"且含"a"或"左"→氨水a的值
                            - 含"氨水"或"铝碱"且含"b"或"右"→氨水b的值
                            - 含"氮气"且含"a"或"左"→氮气a的值
                            - 含"氮气"且含"b"或"右"→氮气b的值
                            - 含"空气"且含"a"或"左"→空气a的值
                            - 含"空气"且含"b"或"右"→空气b的值
                            ...
                            如果列名不含以上任何关键词，输出空字符串""。

                            【输出】严格输出JSON，key必须与模板列名完全一致（一字不差），value只填数字（无单位）或空字符串。不要任何解释。
                            """
