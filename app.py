                    ...
                    with st.spinner(f"正在呼叫 {MODEL_NAME} 视觉大模型，请稍候..."):
                        try:
                            ...
                            regions = {
                                ...
                                "主釜氮气空气": {
                                    "box": (0, 0, int(w*0.45), int(h*0.5)),
                                    "prompt": "这是DCS截图左侧主釜区域，包含多个流量计。\n请找到以下两个流量计并读取PV值（绿底黑字数字，忽略Hz，忽略SP设定值）：\n1. 标注为'氮气'的流量计：读取上面显示的数字\n2. 标注为'空气'的流量计：读取上面显示的数字\n如果某个流量计显示0.0就输出0.0\n只输出JSON，如 {\"氮气主釜\":68.8,\"空气主釜\":0.0}"
                                },
                                "次釜氮气空气": {
                                    "box": (int(w*0.55), 0, w, int(h*0.5)),
                                    "prompt": "这是DCS截图右侧次釜区域，包含多个流量计。\n请找到以下两个流量计并读取PV值（绿底黑字数字，忽略Hz，忽略SP设定值）：\n1. 标注为'氮气'的流量计：读取上面显示的数字\n2. 标注为'空气'的流量计：读取上面显示的数字\n如果某个流量计显示0.0就输出0.0\n只输出JSON，如 {\"氮气次釜\":151.7,\"空气次釜\":0.0}"
                                },
                                "主次釜面板": {
