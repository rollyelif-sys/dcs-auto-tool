                ...
                if st.session_state.last_img == img_id:
                    ...
                    with st.spinner(f"正在呼叫 {MODEL_NAME} 视觉大模型执行工艺数据比对，请稍候..."):
                        try:
                            ...
                            【目标任务】
                            请提取以下字段的数值，并严格按 JSON 格式输出，不要包含单位，只输出纯数字：
                            {formatted_columns}
                            
                            【DCS界面识图指南】：
                            1. 界面结构：左侧较大的反应釜是"主釜"（对应a系列流量），右侧稍小的是"次釜"（对应b系列流量）。
                            2. 数值规则：带有边框的数值框，绿底黑字优先读取（实际值），黑底绿字/白字为设定值。
                            3. 对应关系参考：
                               - a系列（左侧）：金属液a、液碱a、氨水a（可能标注铝碱）、氮气a、空气a。
                               - b系列（右侧）：金属液b、液碱b、氨水b、氮气b、空气b。
                            
                            【⚠️ 特殊工艺规则】：
                            1. pH值：有两个通道。必须读取数值较低的那个实际值（忽略15左右的那个）。
                            2. 氨水：有两个入口。请观察实际流量，只读取有实际流量数值的那个入口的数据。
                            
                            对于"日期"、"时间"、"班次"等无法从屏幕读取的管理字段，请输出空字符串 ""。
                            严格要求：只能输出合法的 JSON 文本，绝对不能包含任何 markdown 标记，不要有多余解释。
                            """
