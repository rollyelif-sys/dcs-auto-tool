                ...
                if st.session_state.last_img == img_id:
                    ...
                    with st.spinner(f"正在呼叫 {MODEL_NAME} 视觉大模型执行工艺数据比对，请稍候..."):
                        try:
                            ...
                            # 核心提示词：精确DCS读取 + 关键词模板映射
                            prompt = f"""
                            你是一个资深的化工DCS巡检员。请从这张SUPCON中控截图中精确提取数据。

                            【读取规则】
                            - 这是双管反应釜界面，左侧是a管（主釜），右侧是b管（次釜）
                            - 只读PV实际值：绿底黑字的大号数字，不读旁边小号的SP设定值
                            - 每个流量计旁边会显示三行数字：第一行是PV流量值，第二行是SP设定值，第三行是频率。只取第一行！
                            - pH有两个通道（pH值1和pH值2），取数值较小的那个（忽略15左右的异常值）
                            - 温度也有两个通道，都读取
                            ...
                            主釜数据面板（屏幕左侧下方"主釜数据"区域）：
                            - pH值 = ___（取两个通道中较小的）
                            - 主釜温度 = ___（取温度1的值）
                            - 主釜液位 = ___
                            - 主釜转速 = ___

                            次釜数据面板（屏幕左侧下方"次釜数据"区域）：
                            - 次釜温度 = ___
                            - 次釜液位 = ___
                            - 次釜转速 = ___
                            ...
                            - 含"液碱"且含"a"或"左" → 液碱a
                            - 含"液碱"且含"b"或"右" → 液碱b
                            - 含"氨水"或"铝碱"且含"a"或"左" → 氨水a
                            - 含"氨水"或"铝碱"且含"b"或"右" → 氨水b
                            - 含"氮气"且含"a"或"左" → 氮气a
                            - 含"氮气"且含"b"或"右" → 氮气b
                            - 含"空气"且含"a"或"左" → 空气a
                            - 含"空气"且含"b"或"右" → 空气b
                            ...
                            【输出】只输出JSON。key必须与模板列名完全一致。value填纯数字（无单位）或空字符串。不要任何解释文字。
                            """
