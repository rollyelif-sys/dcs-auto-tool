                ...
                if st.session_state.last_img == img_id:
                    ...
                    with st.spinner(f"正在呼叫 {MODEL_NAME} 视觉大模型执行工艺数据比对，请稍候..."):
                        try:
                            ...
                            # 核心提示词：两步法——先读DCS原始值，再映射到模板列
                            prompt = f"""
                            你是一个资深的化工DCS巡检员。请仔细读取这张中控屏幕截图。

                            【第一步：读取DCS屏幕上所有可见的工艺参数】
                            请逐个读取屏幕上每个数值框的实际值（绿底黑字），注意区分：
                            - 实际值（PV）：通常是绿底黑字或大号字体
                            - 设定值（SP）：通常是小号字体或不同颜色
                            请识别并记录以下所有可见参数的【实际值】：
                            金属液a流量、液碱a流量、氨水a流量、氮气a流量、空气a流量、
                            金属液b流量、液碱b流量、氨水b流量、氮气b流量、空气b流量、
                            pH值、温度a、温度b、压力a、压力b、转速a、转速b、液位

                            【第二步：映射到模板列】
                            以下是我上传的Excel模板的列名（按顺序）：
                            {formatted_columns}

                            请将第一步读取到的DCS数值，按列名的含义一一对应填入。
                            匹配规则：
                            - 列名含"金属液"+"a"或"左"→金属液a流量
                            - 列名含"液碱"+"a"或"左"→液碱a流量
                            - 列名含"氨水"或"铝碱"→看哪个入口有实际流量就填哪个
                            - 列名含"pH"→取较小的那个pH值（忽略15左右的异常值）
                            - 列名含"温度"且无a/b区分→取主釜温度
                            - 列名含"日期"、"时间"、"班次"→输出空字符串 ""

                            【输出要求】
                            严格按模板列名顺序输出JSON，key必须与模板列名完全一致，value只填纯数字（无单位），管理字段填空字符串。
                            只输出JSON，不要任何解释文字。
                            """
