                ...
                if st.session_state.last_img == img_id:
                    ...
                    with st.spinner(f"正在呼叫 {MODEL_NAME} 视觉大模型执行工艺数据比对，请稍候..."):
                        try:
                            ...
                            # 核心提示词：自适应读取——先扫描屏幕所有数据，再映射模板
                            prompt = f"""
                            你是一个资深的化工DCS巡检员。请读取这张中控屏幕截图，将数据填入Excel模板。

                            【重要规则】
                            1. 不同反应釜的DCS界面布局和字段名可能不同，请先扫描屏幕上实际显示了哪些参数。
                            2. 只读PV（实际值），不读SP（设定值）。PV通常是绿底黑字的大号数字。
                            3. pH有两个通道，取较小值（忽略15左右的异常值）。
                            4. 氨水可能有两个入口（a和b），也可能只有一个有实际流量，分别读取。
                            5. 双管反应釜有a管（主釜）和b管（次釜），两套流量数据都必须读取。
                            6. 有些参数屏幕上有多个数值，取最醒目的那个PV值。

                            【第一步：扫描屏幕，列出所有可见的PV实际值】
                            请按以下格式列出你在屏幕上看到的每一个PV值：
                            金属液a=___  金属液b=___
                            液碱a=___  液碱b=___
                            氨水a=___  氨水b=___
                            氮气a=___  氮气b=___
                            ...
                            将第一步的值按语义匹配到模板列名。匹配不了的管理字段（日期、时间、班次）填空字符串""。

                            【输出】只输出JSON，key与模板列名完全一致。
                            """
