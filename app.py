                ...
                if st.session_state.last_img == img_id:
                    ...
                    with st.spinner(f"正在呼叫 {MODEL_NAME} 视觉大模型执行工艺数据比对，请稍候..."):
                        try:
                            ...
                            prompt = f"""你是化工DCS巡检员，从SUPCON双管反应釜截图提取数据。

【流量计读法规则-最重要！】
每个流量计的数值框里有多行数字，从上到下：
  第一行（大字）= PV实际值，单位L/h或NL/m ← 只读这个！
  第二行（小字）= SP设定值 ← 不读
  第三行 = 频率Hz ← 不读
  最底部（更小的字）= 累计数，单位m³ ← 绝对不读这个！
如果一个数值是另一个的几倍大（如723 vs 223），你肯定读到了累计数，请重新读第一行！

【数据面板读法规则】
屏幕左侧下方有两个绿色数据面板：
"主釜数据"面板：pH值1(约15,不用)、pH值2(较小,用这个)、温度1、温度2、液位、功率、转速
"次釜数据"面板：温度、液位、功率、转速
必须从正确的面板读取！次釜温度在"次釜数据"面板里，不是主釜的！

【读取以下值】
a管流量(左侧): 金属液a=PV值___L/h, 液碱a=PV值___L/h, 氨水a=PV值___L/h
b管流量(右侧): 金属液b=PV值___L/h, 液碱b=PV值___L/h, 氨水b=PV值___L/h
氮气(标签"氮气"): 氮气a=PV值___NL/m, 氮气b=PV值___NL/m
空气(标签"空气"): 空气a=PV值___NL/m, 空气b=PV值___NL/m
主釜面板: pH=较小值___, 主釜温度=___℃, 主釜液位=___m, 主釜转速=___rpm
次釜面板: 次釜温度=___℃, 次釜液位=___m, 次釜转速=___rpm
罐体: 主釜压力=___kPa, 次釜压力=___kPa
循环: 主釜自循环=___L/h, 次釜自循环=___L/h, 主次釜循环=___L/h

【填入模板】列名: {formatted_columns}
同义词: "碱液"="液碱", "搅拌"="转速"
- 含"金属液"且含"A"->金属液a | 含"金属液"且含"B"->金属液b
- 含"碱液"且含"A"->液碱a | 含"碱液"且含"B"->液碱b
- 含"氨"且含"主"->氨水a | 含"氨"且含"次"->氨水b | 含"氨"且无主次->看实际
